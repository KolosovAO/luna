...
async function loadPushConfig(): Promise<PushConfig> {
  ...
  const apiBaseUrl = trimTrailingSlash(fileConfig.apiBaseUrl ?? "");
  let publicVapidKey = fileConfig.publicVapidKey ?? "";
...
async function saveSubscription(apiBaseUrl: string, subscription: PushSubscription): Promise<Response> {
...
export async function getInitialPushState(): Promise<PushRegistrationState> {
  ...
  try {
    ...
    return {
      status: "idle",
      message: "Нажми, чтобы включить напоминание."
    };
  }
